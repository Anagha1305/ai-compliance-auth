...
This merged project keeps the supplied FastAPI/MongoDB authentication backend in
`backend/` and protects every application route behind its HttpOnly JWT cookie.
The frontend sends authentication requests through Vite's `/api` proxy, so the
existing endpoints and payloads remain unchanged.

1. Copy `backend/.env.example` to `backend/.env` and set your MongoDB, SMTP,
   JWT, session, and Google OAuth values. Do not commit this file.
2. For local Google OAuth, set `FRONTEND_URL` to `http://127.0.0.1:5173` and
   register `http://127.0.0.1:8000/api/auth/google/callback` as the Google
   callback URI.
3. Create and activate a Python environment, then run
   `pip install -r backend/requirements.txt` and
   `uvicorn app.main:app --reload --port 8000` from `backend/`.
4. In a second terminal at the project root, run `npm ci` then `npm run dev`.

The auth flow is `/login` → `/signup` → `/verify-email` → protected workspace.
Existing sessions go straight to the workspace; signing out clears the existing
backend cookie and returns to login.
