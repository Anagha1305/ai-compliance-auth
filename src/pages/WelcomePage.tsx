...
function BrandMark() {
  return <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-brand text-sm font-semibold text-white shadow-sm" aria-hidden="true">AI</span>
}
...
export function WelcomePage() {
  return <div className="min-h-svh overflow-hidden bg-bg text-ink">
    <header className="border-b border-border/80 bg-surface/95 backdrop-blur"><div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-5 sm:px-8 lg:px-10"><Link to="/" className="flex min-w-0 items-center gap-3 rounded-md focus-visible:outline-brand"><BrandMark /><span className="truncate text-sm font-semibold text-navy sm:text-base">AI Compliance Inspector</span></Link><nav className="flex items-center gap-1.5" aria-label="Application navigation"><Link to="/dashboard" className="rounded-md px-3 py-2 text-sm font-medium text-muted transition-colors hover:bg-bg hover:text-ink focus-visible:outline-brand">Workspace</Link><Link to="/inspections/new" className="inline-flex h-9 items-center rounded-md bg-brand px-3.5 text-sm font-semibold text-white shadow-sm transition-[background-color,box-shadow,transform] duration-200 hover:-translate-y-px hover:bg-brand-dark hover:shadow-md active:scale-[0.98] focus-visible:outline-brand">Get started</Link></nav></div></header>
    <main>
      <section className="relative overflow-hidden bg-navy text-white">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_18%,rgba(37,99,235,0.32),transparent_34%),linear-gradient(125deg,rgba(255,255,255,0.06),transparent_48%,rgba(22,163,74,0.12))]" aria-hidden="true" />
        ...
        <div className="relative mx-auto max-w-7xl px-5 pb-10 pt-14 sm:px-8 sm:pt-18 lg:px-10 lg:pb-14 lg:pt-20">
          <div className="grid items-center gap-10 lg:grid-cols-[minmax(21rem,0.72fr)_minmax(0,1.28fr)] lg:gap-12">
            <div className="welcome-enter max-w-xl">
              <p className="text-xs font-semibold uppercase text-blue-200">Legal Metrology inspection workflow</p>
              <h1 className="mt-5 text-4xl font-semibold tracking-normal sm:text-5xl lg:text-[4.05rem] lg:leading-[1.04]">Inspect smart.<br /><span className="text-blue-200">Verify compliance.</span></h1>
              <p className="mt-6 max-w-lg text-base leading-7 text-blue-100/82 sm:text-lg">Analyze packaged-commodity labels, detect declaration gaps, calculate automated compliance outcomes, and generate evidence-backed reports.</p>
              <div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link to="/dashboard" className={lightAction}>Open workspace <span aria-hidden="true">-&gt;</span></Link><Link to="/inspections/new" className={outlineAction}>New inspection</Link></div>
              <div className="mt-8 grid grid-cols-3 gap-3 text-center sm:max-w-md">
                <div className="rounded-md border border-white/10 bg-white/[0.06] px-3 py-3"><p className="text-lg font-semibold">OCR</p><p className="mt-1 text-[10px] uppercase text-blue-100/55">Extraction</p></div>
                <div className="rounded-md border border-white/10 bg-white/[0.06] px-3 py-3"><p className="text-lg font-semibold">Rules</p><p className="mt-1 text-[10px] uppercase text-blue-100/55">Evaluation</p></div>
                <div className="rounded-md border border-white/10 bg-white/[0.06] px-3 py-3"><p className="text-lg font-semibold">Score</p><p className="mt-1 text-[10px] uppercase text-blue-100/55">Outcome</p></div>
              </div>
            </div>
          ...
          <div className="mt-10 flex flex-wrap gap-x-6 gap-y-2 border-t border-white/10 pt-5 text-[10px] font-semibold uppercase text-blue-100/58"><span>Packaged commodities</span><span>Product labels</span><span>Automated result</span><span>Evidence</span><span>Reports</span></div>
        </div>
      </section>
      ...
      <section className="bg-navy text-white" aria-labelledby="final-cta-heading"><div className="mx-auto flex max-w-7xl flex-col gap-7 px-5 py-14 sm:px-8 lg:flex-row lg:items-center lg:justify-between lg:px-10 lg:py-16"><div><h2 id="final-cta-heading" className="text-3xl font-semibold tracking-normal">Ready to begin an inspection?</h2><p className="mt-2 max-w-xl text-sm leading-6 text-blue-100/70">Start with a clear product label and move from intake to automated outcome, evidence, and reporting.</p></div><div className="flex flex-col gap-3 sm:flex-row"><Link to="/dashboard" className={lightAction}>Open workspace <span aria-hidden="true">-&gt;</span></Link><Link to="/inspections/new" className={outlineAction}>New inspection</Link></div></div></section>
    </main>
    <footer className="border-t border-white/10 bg-navy px-5 py-6 text-xs text-white/45 sm:px-8 lg:px-10"><div className="mx-auto flex max-w-7xl flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-2"><BrandMark /><span>AI Compliance Inspector</span></div><span>Packaged-commodity compliance inspection workflow.</span></div></footer>
  </div>
}
